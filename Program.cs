    ...
    public class Program
    {
        ...
        private static void HighFive(int[,] items)
        {
            try
            {
                ...
                {
                    Dictionary<int, List<int>> dictOutput = new Dictionary<int, List<int>>();
                    for (int i = 0; i < items.GetLength(0); i++)
                    {
                        if (dictOutput.ContainsKey(items[i, 0]))
                        {
                            List<int> lListNew = dictOutput[items[i, 0]];
                        ...
                        else
                        {
                            ...
                            {
                                val.Add(items[i, 1]);
                                dictOutput.Add(items[i, 0], val);
                            }
                        }
                    }
                    if (dictOutput.Count > 1000 || dictOutput.Count == 1)
                    {
                        throw new Exception();
                    }
                    foreach (KeyValuePair<int, List<int>> llstItem in dictOutput)
                    {
                        if (llstItem.Value.Count < 5)
                        {
                            throw new Exception();
                        }
                        else
                        {
                            var dictValues = llstItem.Value;
                            //Sorting the dictionary
                            dictValues.Sort();
                            //Reversing the dictionary
                            dictValues.Reverse();
                            var reversedictValues = dictValues.GetRange(0, 5);
                            //Printing out the student ID and their respective average.
                            Console.WriteLine((llstItem.Key, reversedictValues.Sum() / 5));
                        }
                    }
                }
                else
